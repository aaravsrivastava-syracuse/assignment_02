...
def summarize_by_item(cleaned_data: list[dict]) -> list[dict]:
    ...
    item_summary = list(totals.values())
    item_summary.sort(key=lambda entry: (-entry["revenue"], entry["item"]))
    return item_summary
...
def summarize_by_day(cleaned_data: list[dict]) -> list[dict]:
    ...
    day_summary = list(totals.values())
    day_summary.sort(key=lambda entry: entry["date"])
    return day_summary
...
def find_top_entry(summary: list[dict], field: str = "revenue") -> dict:
    ...
    top_entry = summary[0]
    # Checking the first entry again is harmless and keeps the loop simple.
